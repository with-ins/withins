...
public class NewsReader {
    ...
    public Page<NewsRequest> search(Pageable pageable, NewsCondition condition) {
        log.debug("전체 게시물 목록 조회 실행 - page={}, size={}", pageable.getPageNumber(), pageable.getPageSize());

        Page<NewsRequest> newsPage = newsQDSLRepository.readAllNewsWithPage(pageable, condition).map(NewsRequest::of);
